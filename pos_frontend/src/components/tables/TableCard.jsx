...
const TableCard = ({id, name, status, initials, seats}) => {
  ...
  return (
    <div onClick={() => handleClick(name)} key={id} className='w-[333px] bg-[#262626] p-4 hover:bg-[#2c2c2c] rounded-lg mb-2 cursor-pointer'>
      ...
      <div className='flex justify-between item-center px-1'>
        ...
        <p className={`${status==="Booked"?"text-green-600 bg-[#2e4a40]":"bg-[#664a04] text-white"} px-4 py-1 rounded-lg`}>{status}</p>
        
      </div>

      <div className='flex item-center justify-center mt-5 mb-7'>
